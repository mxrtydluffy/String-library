...
function kebobCase(str, separator = '-') {
  ...
  const removeSpace = removeExtraSpaces(filtered.join(''));
  return removeSpace.split(' ').join(separator);
}
...
function makeHashTag(str) {
  ...
  const notemtyWords = sortWords.filter((word) => word !== '');
  const hashTags = notemtyWords.slice(0, 3).map((word) => `#${word.toLowerCase()}`);
  return hashTags;
}
